...
int TMIgraph::reportTopology(Moly &moly) {
    ...
    for (node_it = reverse_node_index.begin(); node_it != reverse_node_index.end(); node_it++) {
        ...
        if (node_ID == RVnodeID) {
            ...
            node_type = NODE_TYPE_RV;
            topology_nodes << nodeID_int << "," << node_name << "," << node_type << ", 9" << endl;
            if(!moly.addNodeType(node_name, node_type)) {
        ...
        for (int n = 0; n < igraph_vector_size(&neis); n++) {
            ...
            lid = edge_LID[eid];
            if (lid_to_int.find(lid) == lid_to_int.end()) {
                lid_int = (eid + 1);
                lid_to_int.insert(pair<Bitvector*, int>(lid, lid_int));
            } else {
                lid_int = lid_to_int[lid];
            }
            topology_links << lid_int << "," << nodeID_int << "," << nighbour_nodeID_int << "," << LINK_TYPE_802_3 << ",9" << endl;
            if (!moly.addLink(link_name, lid->to_string(), nodeID_int, nighbour_nodeID_int, LINK_TYPE_802_3)){
                cout << "TM: moly failed to send link"<< nodeID_int << "->" << nighbour_nodeID_int << endl;
                return -1;
            } else
                cout << "TM: link: " << nodeID_int << "-> " << nighbour_nodeID_int << " reported to MA" << endl;
        }
    }
...
bool TMIgraph::updateGraph(const string &source, const string &destination, bool bidirectional, bool remove){
	igraph_vector_t v;
	igraph_es_t es;
