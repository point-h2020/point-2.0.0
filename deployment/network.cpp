				...
				for (map<string, Bitvector>::iterator it = mc_FID.begin();
					...
					for (int j = 1; j < combinations; j++) {
						...
						ss
								...
								<< ipv6_h2 << "\" } } }";
						string flow = ss.str();
						oc.configureFlow(nn->openflow_id, 0, flow,
								flow_counter);
						flow_counter++;
					}
				}
					...
					if (nn->sdn_implementation.compare("tables") == 0) {
						...
						if (j < node_no_connections - 1) {
							...
							ss
									...
									<< "\"output-action\": { \"output-node-connector\": \"" << nc->src_pt << "\" }}}},"
									<< "{\"order\": \"1\",\"go-to-table\": { \"table_id\": \"" << j+1 << "\" }}]}, "
			...
			if (nn->sdn_implementation.compare("bridges") == 0) {
				...
				for (map<string, Bitvector>::iterator it = mc_FID.begin();
					...
					flow_conf << sdn_path << "ovs-ofctl -O OpenFlow13 add-flow "
							<< bridge << " \"dl_dst=00:00:00:00:00:00,"
					...
					if (nn->sdn_implementation.compare("tables") == 0) {
						...
						if (j < node_no_connections - 1) {
							flow_conf
									<< sdn_path << "ovs-ofctl -O OpenFlow13 add-flow "
									<< nn->bridge
									<< " \"dl_dst=00:00:00:00:00:00, table="
									<< j << ", ipv6,priority=500, action="
									<< "resubmit(," << j + 1 << ")\"" << endl;
							flow_conf
									<< sdn_path << "ovs-ofctl -O OpenFlow13 add-flow "
									<< nn->bridge
									<< " \"dl_dst=00:00:00:00:00:00,ipv6_src="
...
void Domain::writeClickFiles(bool montoolstub, bool dump_supp, bool no_lnxcap) {
	ofstream click_conf;
    ...
    for (size_t i = 0; i < network_nodes.size(); i++) {
        ...
        if(nn->operating_system.compare("ovs") != 0){
            ...
            if (nn->running_mode.compare("user") == 0) {
                ...
                if (sudo) {
                    command = "ssh -o \"StrictHostKeyChecking no\" -o ConnectTimeout=5 " + user + "@" + nn->testbed_ip + " \"screen -d -m -S mona sudo ~/blackadder/apps/monitoring/agent/mona\"";
                } else {
                    command = "ssh -o \"StrictHostKeyChecking no\" -o ConnectTimeout=5 " + user + "@" + nn->testbed_ip + " \"screen -d -m -S mona sudo ~/blackadder/apps/monitoring/agent/mona\"";
                }
                cout << command << endl;
            ...
            if (nn->running_mode.compare("user") == 0) {
                if (sudo) {
                    command = "ssh -o \"StrictHostKeyChecking no\" -o ConnectTimeout=5 " + user +
                    		"@" + nn->testbed_ip + " screen -d -m -S nap sudo nap";
                } else {
                    command = "ssh -o \"StrictHostKeyChecking no\" -o ConnectTimeout=5 " + user +
                    		"@" + nn->testbed_ip + " screen -d -m -S nap sudo nap";
                }
