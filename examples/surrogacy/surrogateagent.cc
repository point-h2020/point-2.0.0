...
int main(int argc, char* argv[]) {
	if (argc != 4)
	{
		cout << "surrogateagent <FQDN> <IP_SERVER> 0||1\n\n"
				"Three arguments are required: FQDN, IP address of server and "
				"numeric value (0 or 1) where 0 represents 'deactivate' and 1 "
				"'activate'" << endl;
		return EXIT_FAILURE;
	}
    uint32_t fqdn;
    ...
    argvCommand = atoi(argv[3]);
    if (argvCommand != 1 && argvCommand != 0)
    {
    	cout << "Third argument must be 1 (activate) or 0 (deactivate)\n";
    	return EXIT_FAILURE;
    }
	...
	nlh->nlmsg_len = NLMSG_SPACE(sizeof(uint32_t) + sizeof(uint32_t));
	...
	memcpy(NLMSG_DATA(nlh), &fqdn, sizeof(uint32_t));
	memcpy((uint8_t *)NLMSG_DATA(nlh) + sizeof(uint32_t), &ipAddress,
			sizeof(uint32_t));
	// Header
	...
	{
		cout << "Message successfully sent to NAP" << endl;
	}
