		...
		try
		{
			if (exists(p) && is_directory(p))
			{
				...
				if (oss.str().length() != 0)
				{
					...
					cid.append(_nodeType(dataPoint));
					...
					// data point
					cid.append(_attribute(dataPoint));
...
BampersInformationItems FilesystemChecker::_attribute(uint16_t dataPoint)
{
	BampersInformationItems infoItem;
	...
	case ATTRIBUTE_NAP_HTTP_REQUESTS:
		infoItem = II_HTTP_REQUESTS_PER_FQDN;
		break;
	...
	case ATTRIBUTE_NAP_NETWORK_LATENCY:
		infoItem = II_NETWORK_LATENCY_PER_FQDN;
		break;
	...
	case ATTRIBUTE_LNK_BYTES_PER_TIME_INTERVAL:
...
NodeTypes FilesystemChecker::_nodeType(uint16_t dataPoint)
{
	NodeTypes nodeType;
	...
	case ATTRIBUTE_TM_CPU_THREADS:
	case ATTRIBUTE_TM_STATE:
		nodeType = NODE_TYPE_TM;
		break;
	case ATTRIBUTE_UE_NAME:
	case ATTRIBUTE_UE_DASH_JITTER:
	case ATTRIBUTE_UE_DASH_DROPPED_FRAMES:
	case ATTRIBUTE_UE_STATE:
		nodeType = NODE_TYPE_UE;
		break;
	case ATTRIBUTE_LNK_DESTINATIONS:
	case ATTRIBUTE_LNK_BYTES_PER_TIME_INTERVAL:
	case ATTRIBUTE_LNK_STATE:
