...
{
	_tcpListener = -1;
	_tcpServerThreads = new boost::thread_group;
}
...
HttpProxy::~HttpProxy()
{
	if (_tcpListener != -1)
	{
		LOG4CXX_INFO(logger, "Closing TCP listener");
		close(_tcpListener);
	}
}
...
void HttpProxy::tearDown()
{
	...
	iptablesRule << "iptables -D PREROUTING -t nat -p tcp -i "
			<< _configuration.networkDevice() << " --dport 80 -j REDIRECT "
...
void HttpProxy::operator()()
{
	...
	{
		backLog = atoi(ptr);
		LOG4CXX_INFO(logger, "Maximum number of TCP sessions is " << backLog);
	}

	if (listen(_tcpListener, backLog) == -1)
	{
		LOG4CXX_FATAL(logger, "Cannot listen on socket");
		return;
	}
	...
	while(true)
	{
		...
		else
		{
			...
			TcpServer tcpServer(_configuration, _namespaces, newSocketFd,
					ipAddress);
			_tcpServerThreads->create_thread(tcpServer);
		}
	}

	LOG4CXX_DEBUG(logger, "Closing TCP listening socket on proxy port "
			<< _configuration.httpProxyPort());
	close(_tcpListener);
}
